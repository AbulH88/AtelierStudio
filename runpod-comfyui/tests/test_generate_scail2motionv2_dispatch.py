...
def test_generate_scail2motionv2_returns_both_outputs(monkeypatch):
    calls = {}
    ...
    inp = {"mode": "scail2motionv2", "prompt": "p", "video_b64": _B64, "ref_b64": _B64,
           "upscale": True, "seed": 7}
...
def test_generate_scail2motionv2_raw_only_when_upscale_off(monkeypatch):
    monkeypatch.setattr(cc, "upload_video", lambda base, raw, filename="driving.mp4": "drv.mp4")
    monkeypatch.setattr(cc, "upload_image", lambda base, raw: "ref.png")
    ...
    inp = {"mode": "scail2motionv2", "prompt": "p", "video_b64": _B64, "ref_b64": _B64, "seed": 3}
    out = cc.generate("http://fake", WORKFLOW_DIR, inp)
    assert out == {"videos": ["RAWb64"], "seed": 3}


def test_generate_scail2motionv2_no_video_produced_is_an_error(monkeypatch):
    monkeypatch.setattr(cc, "upload_video", lambda base, raw, filename="driving.mp4": "drv.mp4")
    monkeypatch.setattr(cc, "upload_image", lambda base, raw: "ref.png")
