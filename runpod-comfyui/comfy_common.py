...
I2I = {"load_image": "1", "qwen": "2", "face_concat": "4", "negative": "6",
...
def _build_i2i(graph, inp, seed, frame_name):
    ...
    graph[nm["face_concat"]]["inputs"]["string_a"] = FACE_PREFIX
    graph[nm["negative"]]["inputs"]["text"] = NEGATIVE
    # optional: override the QwenVL caption instruction from the UI
    cap = (inp.get("caption_prompt") or "").strip()
    if cap:
        graph[nm["qwen"]]["inputs"]["custom_prompt"] = cap
